...
impl GitHubAuthenticator {
    ...
    pub async fn ensure_authenticated(&self) -> GistResult<Arc<Octocrab>> {
        ...
        // Try to create client from stored token first
        if let Ok(()) = self.try_from_stored_token().await {
            debug!("Successfully created GitHub client from stored keychain token");
            let client_guard = self.client.read().await;
            return Ok(Arc::new(client_guard.as_ref().unwrap().clone()));
        }

        // Need to perform fresh OAuth authentication
        info!("No valid stored token found, performing GitHub OAuth...");
