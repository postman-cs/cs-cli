...
use crate::api::customer::CustomerCallInfo;
use crate::api::customer::GongCustomerSearchClient;
use crate::api::email::EmailEnhancer;
use crate::api::library::{CallDetailsFetcher, GongLibraryClient, LibraryCallInfo};
use crate::api::timeline::TimelineExtractor;
use crate::auth::GongAuthenticator;
use crate::config::AppConfig;
use crate::models::{Call, CallDirection, Email};
use crate::output::markdown::{CallMarkdownFormatter, CallSummaryReporter};
use crate::Result;
...
fn convert_library_call_info_to_call(info: LibraryCallInfo) -> Call {
    use crate::models::CallParticipant;
...
pub fn save_config(config: &CliConfig) -> Result<()> {
    ...
    let contents = serde_json::to_string_pretty(config).map_err(|e| {
        crate::error::CsCliError::Configuration(format!("Failed to serialize config: {e}"))
    })?;

    fs::write(&config_path, contents)
        .map_err(|e| crate::error::CsCliError::FileIo(format!("Failed to save config: {e}")))?;
...
impl TeamCallsExtractor {
    ...
    pub async fn setup(&mut self) -> Result<()> {
        println!("{}", style("Setting up CS-CLI extractor...").cyan());
        ...
        if !authenticated {
            return Err(crate::error::CsCliError::Authentication(
    ...
    pub async fn extract_team_calls(
        ...
        let library_client = self.library_client.as_ref().ok_or_else(|| {
            crate::error::CsCliError::Generic("Library client not initialized".to_string())
        })?;
        ...
        let details_fetcher = self.details_fetcher.as_ref().ok_or_else(|| {
            crate::error::CsCliError::Generic("Details fetcher not initialized".to_string())
        })?;
    ...
    pub async fn extract_customer_calls(
        ...
        let customer_client = self.customer_search_client.as_ref().ok_or_else(|| {
            crate::error::CsCliError::Generic("Customer search client not initialized".to_string())
        })?;
    ...
    pub async fn extract_customer_communications(
        ...
        let timeline_extractor = self.timeline_extractor.as_mut().ok_or_else(|| {
            crate::error::CsCliError::Generic("Timeline extractor not initialized".to_string())
        })?;
        ...
        if (include_emails || emails_only) && fetch_email_bodies && !emails.is_empty() {
            if let Some(email_enhancer) = &self.email_enhancer {
                println!("{}", style("Fetching email body content...").cyan());
                ...
                emails = email_enhancer
                    ...
                    .await
                    .map_err(|e| {
                        crate::error::CsCliError::Generic(format!("Failed to enhance emails: {e}"))
                    })?;

                pb.finish_with_message("Email body enhancement complete");
                println!("{}", style("Email body enhancement complete").green());
            }
        }
    ...
    pub fn save_calls_as_markdown(
        ...
        println!("{}", style("Generating markdown files...").cyan());

        let saved_files = self
            .formatter
            .save_multiple_calls(calls, customer_name)
            .map_err(|e| crate::error::CsCliError::Generic(format!("Failed to save calls: {e}")))?;
        ...
        if let Some(first_file) = saved_files.first() {
            if let Some(output_dir) = first_file.parent() {
                ...
                self.summary_reporter
                    ...
                    .map_err(|e| {
                        crate::error::CsCliError::Generic(format!(
    ...
    pub fn save_emails_as_markdown(
        ...
        println!("{}", style("Generating email markdown files...").cyan());

        let saved_files = self
            .formatter
            .save_emails_as_markdown(emails, customer_name, Some(customer_name))
            .map_err(|e| {
                crate::error::CsCliError::Generic(format!("Failed to save emails: {e}"))
            })?;
...
async fn execute_command(
    ...
    match command {
        ParsedCommand::Team {
            ...
            let stream_id = match stream_id {
                ...
                None => {
                    ...
                    } = team_command
                    {
                        ...
                        return Err(crate::error::CsCliError::Generic(
            ...
            // Display results
            println!();
            println!("{}", style("Extraction Complete!").bold().green());
