...
use crate::common::auth::github::github_gist_storage::GitHubGistStorage;
...
impl HybridCookieStorage {
    ...
    pub async fn get_cookies(&self) -> Result<HashMap<String, String>> {
        ...
        if let Some(ref gist_storage) = self.gist_storage {
            match gist_storage.get_cookies().await {
                ...
                Err(e) => {
                    // Only warn for non-configuration errors to reduce noise
                    if !e.to_string().contains("Configuration error") {
                        warn!("Failed to retrieve from gist, trying local: {}", e);
                    } else {
                        debug!("Gist storage not configured, using local storage: {}", e);
                    }
                }
            }
        }
    ...
    /// Delete cookies from all storage locations (preserves GitHub authentication)
    pub async fn delete_cookies(&mut self) -> Result<()> {
        let mut errors = Vec::new();

        // Delete from gist storage (preserves GitHub token)
        if let Some(ref mut gist_storage) = self.gist_storage {
    ...
    /// Delete all authentication data including GitHub tokens (for full reset)
    pub async fn delete_all_authentication(&mut self) -> Result<()> {
        let mut errors = Vec::new();

        // Delete from gist storage (clears GitHub token too)
        if let Some(ref mut gist_storage) = self.gist_storage {
            if let Err(e) = gist_storage.delete_all_authentication().await {
                errors.push(format!("Gist deletion failed: {e}"));
            }
        }
        ...
        if errors.is_empty() {
            info!("All authentication data deleted from all storage locations");
            Ok(())
        } else {
            warn!("Some deletion operations failed: {:?}", errors);
            Ok(()) // Non-fatal - best effort cleanup
...
/// Delete all authentication data including GitHub tokens (for full reset)
pub async fn delete_all_authentication_hybrid() -> Result<()> {
    let mut storage = HybridCookieStorage::new().await?;
    storage.delete_all_authentication().await
}
