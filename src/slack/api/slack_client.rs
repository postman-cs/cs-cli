...
impl SlackClient {
    ...
    pub async fn auth_test(&self) -> Result<Value> {
        ...
        let session = self.auth.get_session_data()
    ...
    pub async fn test_basic_functionality(&mut self) -> Result<()> {
        ...
        if auth_info.get("ok").and_then(|v| v.as_bool()).unwrap_or(false) {
            ...
            return Err(CsCliError::Authentication("auth.test returned ok=false".to_string()));
        }
