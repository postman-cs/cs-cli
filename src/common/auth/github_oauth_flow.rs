...
impl GitHubOAuthFlow {
    ...
    fn open_browser(&self, url: &str) -> Result<()> {
        ...
        let result = if cfg!(target_os = "macos") {
            ...
            return Err(CsCliError::GitHubOAuth(
                "Unsupported operating system for browser launch".to_string(),
            ));
        };

        result.map_err(|e| CsCliError::GitHubOAuth(format!("Failed to open browser: {e}")))?;
    ...
    async fn start_callback_server(&self) -> Result<TcpListener> {
        let listener = TcpListener::bind("127.0.0.1:8080").await.map_err(|e| {
            CsCliError::GitHubOAuth(format!(
                "Failed to bind OAuth callback server - port 8080 may be in use: {e}"
            ))
        })?;

        debug!("OAuth callback server listening on http://localhost:8080");
        Ok(listener)
    }
...
<html>
<head>
    <title>CS-CLI Authorization Complete</title>
    <style>
...
<body>
    ...
    <div class="instruction">You can close this window and return to cs-cli.</div>
</body>
</html>"#;
    ...
    async fn exchange_code_for_token(&self, auth_code: &str) -> Result<String> {
        #[derive(Deserialize)]
