...
use crate::common::http::{channel_delay, search_delay, BrowserHttpClient, HttpClient};
...
impl SlackExplorer {
    ...
    pub async fn get_conversation_messages(
        ...
        let json: Value = response.json().await.map_err(|e| {
            CsCliError::ApiRequest(format!("Failed to parse messages response: {}", e))
        })?;
    ...
    pub async fn search_messages(&self, query: &str, count: u32) -> Result<Vec<Value>> {
        ...
        let json: Value = response.json().await.map_err(|e| {
            CsCliError::ApiRequest(format!("Failed to parse search response: {}", e))
        })?;

        let mut messages = Vec::new();

        if let Some(search_results) = json
            .get("messages")
    ...
    pub async fn analyze_conversations(&self) -> Result<ConversationAnalysis> {
        ...
        for conv in &conversations {
            ...
            if let Some(name) = &conv.name {
                ...
                if name_lower.contains("engineering")
                ...
                if name_lower.contains("general")
                    || name_lower.contains("random")
        ...
        println!(
            "      👥 Customer-related: {}",
            analysis.customer_related_channels.len()
        );
        println!(
            "      🔧 Engineering: {}",
            analysis.engineering_channels.len()
        );
    ...
    pub async fn explore_customer_channels(
        &self,
        analysis: &ConversationAnalysis,
    ) -> Result<Vec<ChannelExploration>> {
    ...
    pub async fn explore_workspace(&mut self) -> Result<WorkspaceExploration> {
        ...
        println!(
            "📍 Workspace: {}",
            workspace_info.get("team").unwrap_or(&"Unknown".to_string())
        );
        println!(
            "👤 User: {}",
            workspace_info.get("user").unwrap_or(&"Unknown".to_string())
        );
        println!();
        ...
        let channel_explorations = self
            .explore_customer_channels(&conversation_analysis)
            .await?;
        println!();
