...
//!
//! ```rust
//! use crate::common::auth::github_oauth_config::{GitHubOAuthConfig, OAuthState};
//!
...
impl GitHubOAuthConfig {
    ...
    fn validate_client_id(client_id: &str) -> Result<()> {
        if client_id.is_empty() {
            return Err(CsCliError::Configuration("Client ID cannot be empty".to_string()));
        }
        if client_id.len() < 8 {
            return Err(CsCliError::Configuration("Client ID too short".to_string()));
        }
        if client_id.len() > 64 {
            return Err(CsCliError::Configuration("Client ID too long".to_string()));
        }
        Ok(())
    }

    /// Validate client secret format
    fn validate_client_secret(client_secret: &str) -> Result<()> {
        if client_secret.is_empty() {
            return Err(CsCliError::Configuration("Client secret cannot be empty".to_string()));
        }
        if client_secret.len() < 16 {
            return Err(CsCliError::Configuration("Client secret too short".to_string()));
        }
        if client_secret.len() > 128 {
            return Err(CsCliError::Configuration("Client secret too long".to_string()));
        }
        Ok(())
    }
    ...
    fn validate_callback_url(url: &str) -> Result<()> {
        ...
        // Only allow localhost or HTTPS URLs for security
        if !url.starts_with("http://localhost:") && !url.starts_with("https://") {
            return Err(CsCliError::Configuration(
                "Callback URL must be localhost or HTTPS for security".to_string()
            ));
        }
...
impl OAuthState {
    ...
    pub fn validate_format(state: &str) -> Result<()> {
        if state.is_empty() {
            return Err(CsCliError::GitHubOAuth("OAuth state parameter cannot be empty".to_string()));
        }
        if state.len() < 16 {
            return Err(CsCliError::GitHubOAuth("OAuth state parameter too short".to_string()));
        }
        if state.len() > 128 {
            return Err(CsCliError::GitHubOAuth("OAuth state parameter too long".to_string()));
        }
...
pub fn build_oauth_url(config: &GitHubOAuthConfig, state: &str) -> Result<String> {
...
mod tests {
    ...
    fn test_oauth_url_invalid_state() {
        ...
        let result = build_oauth_url(&config, "");
