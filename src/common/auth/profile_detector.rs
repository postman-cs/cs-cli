...
//! for platform-specific authentication. It can detect Chrome and Firefox profiles,
//! parse their metadata, and intelligently select the best profile for a given platform.
...
impl BrowserProfile {
    ...
    pub fn is_work_profile(&self) -> bool {
        if let Some(email) = &self.email {
            // Check for common work email domains
    ...
    pub fn matches_platform(&self, platform: &PlatformType) -> bool {
        match platform {
            ...
            PlatformType::Gainsight => {
                // Gainsight could be either work or personal, prefer work
                true
            }
        }
    }
...
impl ProfileDetector {
    ...
    /// Discover Chrome profiles by parsing Local State
    ...
    /// Parse a single Chrome profile from JSON data
    fn parse_chrome_profile(
        ...
        // Get last active time from active_time field (Chrome timestamp format)
        let last_active = profile_data["active_time"].as_f64()
            .and_then(|timestamp| {
                // Chrome uses epoch time in seconds as float
    ...
    /// Discover Firefox profiles by scanning the profiles directory
    fn discover_firefox_profiles() -> Result<Vec<BrowserProfile>> {
        ...
        for entry in entries.flatten() {
            if entry.path().is_dir() {
                ...
                // Skip hidden directories and invalid profiles
    ...
    /// Parse a single Firefox profile
    fn parse_firefox_profile(profile_name: &str, profile_path: &Path) -> Result<BrowserProfile> {
        // Extract last active time from profile directory modification time
        let last_active = fs::metadata(profile_path)
            .ok()
            .and_then(|metadata| metadata.modified().ok());
            
        // Firefox profile names are typically in format: randomstring.profilename
        ...
        Ok(BrowserProfile {
            ...
            email: None, // Firefox doesn't store email in easily accessible format
            profile_path: profile_path.to_path_buf(),
            last_active,
            is_managed: false, // Firefox management detection would require additional parsing
            hosted_domain: None,
        })
    }
    ...
    pub fn find_best_profile_for_platform(&self, platform: &PlatformType) -> Option<&BrowserProfile> {
        ...
        if candidates.is_empty() {
            // If no platform-specific matches, return most recently active
            return self.detected_profiles
                .iter()
                .max_by_key(|profile| profile.last_active);
        }
        
        // Sort by preference: Chrome first, then by last active time
        candidates.sort_by(|a, b| {
            // Prefer Chrome over Firefox
            match (&a.browser_type, &b.browser_type) {
                (BrowserType::Chrome, BrowserType::Firefox) => std::cmp::Ordering::Less,
                (BrowserType::Firefox, BrowserType::Chrome) => std::cmp::Ordering::Greater,
                _ => {
                    // Same browser type, sort by last active time (most recent first)
    ...
    /// Find work-related profiles (managed or with work email domains)
    ...
    /// Find personal profiles (non-managed, personal email domains)
    ...
    /// Get Chrome application support directory
